...
app.get('/user/:username', (req, res) => {
  ...
  const postsQuery = `SELECT id FROM posts WHERE user_id=${user}`;
  const commentsQuery = `SELECT id FROM comments_posts WHERE user_id=${user}`;
  const votesQuery = `SELECT * FROM users WHERE id=${user}`;
...
app.get('/user/:username/posts', (req, res) => {
  const queryString = `SELECT * FROM posts WHERE user_id=${req.query.user_id}`;
...
app.patch('/:post_id/upvote', (req, res) => {
  const queryString = `UPDATE posts SET votes = votes + 1 WHERE id=${req.params.post_id} RETURNING votes`;
